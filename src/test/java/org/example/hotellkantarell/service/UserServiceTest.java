...
class UserServiceTest {
    ...
    @Test
    void editProfile_updatesUserData() {
        UserDto userDto = new UserDto(1L, "Old Name", "old@example.com");
        User user = new User();
        user.setId(1L);
    ...
    @Test
    void editPassword_withValidPassword_returnsTrue_andAddsSuccessFlash() {
        UserDto userDto = new UserDto(1L, "User", "user@example.com");
        EditPasswordRequest request = new EditPasswordRequest("newpassword");
        ...
        boolean result = userService.editPassword(userDto, request, redirectAttributes, session);

        assertTrue(result);
        verify(session).setAttribute(eq("user"), any(UserDto.class));
        verify(redirectAttributes).addFlashAttribute("success", "Ändrade lösenordet!");
    }

    @Test
    void editPassword_withShortPassword_returnsFalse_andAddsErrorFlash() {
        UserDto userDto = new UserDto(1L, "User", "user@example.com");
        EditPasswordRequest request = new EditPasswordRequest("short");
        ...
        assertFalse(result);
        verify(redirectAttributes).addFlashAttribute("error", "Lösenordet måste vara minst 6 karaktärer.");
